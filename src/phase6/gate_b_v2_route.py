...
_V2_RESERVATION_AUTHORIZATION_LOCK = threading.RLock()
_V2_LIFECYCLE_REGISTRY_MISSING = object()
_V2_PLAN_CLOSE_PROVENANCE_ERROR = "v2 execution-plan close provenance mismatch"
_V2_ROUTE_LIFECYCLE_UNAVAILABLE_ERROR = "v2 prepared route is not available"
...
def _retire_gate_b_v2_pinned_spec_reference(reference: object) -> None:
    """Remove the one-shot bootstrap capability regardless of outcome."""
    if type(reference) is GateBV2PinnedSpecReference:
        with _V2_RESERVATION_AUTHORIZATION_LOCK:
            _PINNED_SPEC_REGISTRY.pop(id(reference), None)
...
def _load_stored_artifact(
    ...
    *,
    ...
    try:
        ...
        _ARTIFACT_REGISTRY.pop(id(snapshot), None)
        object.__setattr__(snapshot, "raw", b"")
        object.__setattr__(snapshot, "size_bytes", 0)
        raise
...
def _new_runtime_root_reference(
    ...
    try:
        ...
        _ROOT_REF_REGISTRY.pop(id(ref), None)
        object.__setattr__(ref, "_payload", MappingProxyType({}))
        object.__setattr__(ref, "_fixed_root", MappingProxyType({}))
        object.__setattr__(ref, "_descriptor", MappingProxyType({}))
        object.__setattr__(ref, "_anchor_raw", None)
        raise
...
def _weak_reference_targets(entry: object, expected: object) -> bool:
    if type(entry) is not weakref.ReferenceType:
        return False
    try:
...
def _authorization_entry_matches(
    entry: object,
    request: GateBLoaderRequest,
    ...
    try:
        registered_route = _PREPARED_REGISTRY.get(id(route))
        return (
            _weak_reference_targets(reference, request)
            and type(route) is PreparedGateBV2ExecutionRoute
            and type(registered_route) is tuple
            and len(registered_route) == 9
            and registered_route[0] is route
            and registered_route[1] is plan
            and not registered_route[5]
...
def _close_v2_request_lifecycle(
    plan: GateBV2ExecutionPlan,
    request: GateBLoaderRequest,
    *,
    allow_unregistered_runtime_plan: bool,
) -> bool:
    """Atomically detach and validate all strong request-lifecycle registry edges."""
    request_id = id(request)
    with _V2_RESERVATION_AUTHORIZATION_LOCK:
        origin = _V2_RUNTIME_REQUEST_ORIGINS.pop(
            request_id,
            _V2_LIFECYCLE_REGISTRY_MISSING,
        )
        authorization = _V2_RESERVATION_AUTHORIZATIONS.pop(
            request_id,
            _V2_LIFECYCLE_REGISTRY_MISSING,
        )
        runtime_plan = _V2_RUNTIME_REQUEST_PLANS.pop(
            request_id,
            _V2_LIFECYCLE_REGISTRY_MISSING,
        )
        _V2_RUNTIME_REQUEST_COPY_PROVENANCE.pop(request_id, None)
        try:
            state = request._v2_reservation_state
            origin_marker = request._v2_reservation_origin
            authorization_marker = request._v2_reservation_authorization
        except BaseException:
            state = None
            origin_marker = None
            authorization_marker = None
        state_is_planned = type(state) is str and state == "planned"
        state_is_authorized = type(state) is str and state == "authorized"
        state_is_consumed = type(state) is str and state == "consumed"
        origin_matches = _weak_reference_targets(origin, request)
        plan_matches = runtime_plan is plan or (
            allow_unregistered_runtime_plan
            and state_is_planned
            and runtime_plan is _V2_LIFECYCLE_REGISTRY_MISSING
        )
        if state_is_planned:
            ...
            authorization_matches = (
                _authorization_entry_matches(authorization, request, plan)
                and authorization_marker is _V2_RESERVATION_AUTHORIZATION_TOKEN
            )
        ...
        try:
            object.__setattr__(request, "_v2_reservation_state", "closed")
            object.__setattr__(request, "_v2_reservation_authorization", None)
        except BaseException:
            return False
        return (
            (state_is_planned or state_is_authorized or state_is_consumed)
            and origin_marker is _V2_RUNTIME_REQUEST_TOKEN
            and origin_matches
            and plan_matches
...
def close_gate_b_v2_execution_plan(
    plan: GateBV2ExecutionPlan,
    *,
...
def _close_gate_b_v2_execution_plan_locked(
    plan: GateBV2ExecutionPlan,
    *,
    allow_unregistered_runtime_plan: bool,
) -> None:
    ...
    try:
        ...
        closed_tombstone_matches = False
    if registered_entry is None and closed_tombstone_matches:
        return
    try:
        current = _plan_snapshot(plan)
    except BaseException:
    ...
    registered_is_owned_snapshot = (
        type(registered_entry) is tuple
        and len(registered_entry) == 20
        and registered_entry[0] is plan
        and type(registered_entry[8]) is tuple
        and len(registered_entry[8]) == 23
        and type(registered_entry[15]) is tuple
        and type(registered_entry[16]) is tuple
    )
    if registered_is_owned_snapshot:
        registered = registered_entry
    elif current is not None:
        registered = current
        validation_error = True
    else:
        _PLAN_REGISTRY.pop(id(plan), None)
        _fail(_V2_PLAN_CLOSE_PROVENANCE_ERROR)
    if current is None or registered != current or plan._token is not _PLAN_TOKEN:
        validation_error = True
    first_error: BaseException | None = None
    ...
    if not _close_v2_request_lifecycle(
        plan,
        registered_request,
        allow_unregistered_runtime_plan=allow_unregistered_runtime_plan,
    ):
        validation_error = True
...
def _cleanup_unregistered_v2_plan_graph(
    *,
    plan: GateBV2ExecutionPlan | None,
    compatibility_chain: object,
    bundle_evidence: object,
    artifacts: Mapping[str, GateBV2StoredArtifactSnapshot],
    root_references: Mapping[str, GateBV2RuntimeRootReference],
    batch: GateBBatchManifest | None,
    readiness: GateBReadinessAuthorization | None,
    context: GateBExecutionContext | None,
    request: GateBLoaderRequest | None,
    input_owners: tuple[_GateBV2RetainedInputOwner, ...],
) -> None:
    """Best-effort erasure for every object created before plan registration."""
    with _V2_RESERVATION_AUTHORIZATION_LOCK:
        if type(request) is GateBLoaderRequest:
            request_id = id(request)
            _V2_RUNTIME_REQUEST_ORIGINS.pop(request_id, None)
            _V2_RUNTIME_REQUEST_PLANS.pop(request_id, None)
            _V2_RUNTIME_REQUEST_COPY_PROVENANCE.pop(request_id, None)
            _V2_RESERVATION_AUTHORIZATIONS.pop(request_id, None)
            for name, value in (
                ("roots", MappingProxyType({})),
                ("_payload", MappingProxyType({})),
                ("_v2_reservation_origin", None),
                ("_v2_reservation_state", "closed"),
                ("_v2_reservation_authorization", None),
            ):
                with suppress(BaseException):
                    object.__setattr__(request, name, value)

        for root_reference in tuple(root_references.values()):
            _ROOT_REF_REGISTRY.pop(id(root_reference), None)
            for name, value in (
                ("_payload", MappingProxyType({})),
                ("_fixed_root", MappingProxyType({})),
                ("_descriptor", MappingProxyType({})),
                ("_anchor_raw", None),
            ):
                with suppress(BaseException):
                    object.__setattr__(root_reference, name, value)
        ...
        if type(bundle_evidence) is ValidatedPhase6ContractBundleEvidence:
            contracts_module._BUNDLE_EVIDENCE_REGISTRY.pop(id(bundle_evidence), None)
            try:
                bundle_artifacts = tuple(bundle_evidence._artifacts)
            except BaseException:
                bundle_artifacts = ()
            for artifact in bundle_artifacts:
                with suppress(BaseException):
                    object.__setattr__(artifact, "raw", b"")
            for name, value in (
                ("_root_manifest_raw", b""),
                ("_artifacts", ()),
                ("_loader_token", None),
            ):
                with suppress(BaseException):
                    object.__setattr__(bundle_evidence, name, value)

        if type(compatibility_chain) is GateBV2CompatibilityTrustChain:
            gate_b_contracts_module._V2_TRUST_CHAIN_REGISTRY.pop(
                id(compatibility_chain),
                None,
            )
            try:
                projection = compatibility_chain.projection
            except BaseException:
                projection = None
            if projection is not None:
                gate_b_contracts_module._V2_PROJECTION_REGISTRY.pop(id(projection), None)
                for name, value in (
            ...
            for name, value in (
                ...
                ("_artifact_raws", MappingProxyType({})),
            ):
                with suppress(BaseException):
                    object.__setattr__(compatibility_chain, name, value)

        for value in (batch, readiness, context):
            if value is not None:
                with suppress(BaseException):
                    object.__setattr__(value, "_payload", MappingProxyType({}))
                    object.__setattr__(value, "_raw", b"")

        for owner in reversed(input_owners):
            with suppress(BaseException):
                _close_input_owner(owner)

        if type(plan) is GateBV2ExecutionPlan:
            _PLAN_ROUTE_OWNERS.pop(id(plan), None)
            _PLAN_REGISTRY.pop(id(plan), None)
            for name, value in (
                ("compatibility_chain", None),
                ("phase6_contract_bundle_evidence", None),
                ("projection_descriptor", MappingProxyType({})),
                ("artifact_hashes", MappingProxyType({})),
                ("roots", MappingProxyType({})),
                ("request", None),
...
def build_gate_b_v2_execution_plan(
    ...
    *,
    ...
    plan: GateBV2ExecutionPlan | None = None
    artifacts: dict[str, GateBV2StoredArtifactSnapshot] = {}
    runtime_roots: dict[str, GateBV2RuntimeRootReference] = {}
    batch: GateBBatchManifest | None = None
    readiness_object: GateBReadinessAuthorization | None = None
    context_object: GateBExecutionContext | None = None
    request_object: GateBLoaderRequest | None = None
    cleanup_input_owners = tuple(
        owner
        for owner in (*_additional_input_owners, _retained_input_owner)
        if type(owner) is _GateBV2RetainedInputOwner
    )
    try:
        ...
        if _retained_input_owner is None:
            ...
            cleanup_input_owners = (input_owner,)
        ...
        cleanup_input_owners = input_owners
        if (
            not input_owners
        ...
        if (
            ...
            or type(request_object._v2_reservation_state) is not str
        ...
        if registered_plan is not None:
            with suppress(Exception):
                close_gate_b_v2_execution_plan(
                    registered_plan,
                    _allow_unregistered_runtime_plan=True,
                )
        else:
            _cleanup_unregistered_v2_plan_graph(
                plan=plan,
                compatibility_chain=compatibility_chain,
                bundle_evidence=phase6_contract_bundle_evidence,
                artifacts=artifacts,
                root_references=runtime_roots,
                batch=batch,
                readiness=readiness_object,
                context=context_object,
                request=request_object,
                input_owners=cleanup_input_owners,
            )
        raise
...
def is_gate_b_v2_runtime_request(value: object) -> bool:
    ...
    if type(value) is GateBLoaderRequest:
        with _V2_RESERVATION_AUTHORIZATION_LOCK:
            ...
            copied = _V2_RUNTIME_REQUEST_COPY_PROVENANCE.get(id(value))
            registered_v2 = _weak_reference_targets(
                origin,
                value,
            ) or _weak_reference_targets(copied, value)
    try:
        marked_v2 = (
...
def _inherit_gate_b_v2_request_copy_provenance(
    source: GateBLoaderRequest,
    duplicate: GateBLoaderRequest,
) -> None:
    """Register a v2-derived copy without trusting copied fields or payload."""
    if type(source) is not GateBLoaderRequest or type(duplicate) is not GateBLoaderRequest:
        _fail("v2 request copy provenance nominal mismatch")
    with _V2_RESERVATION_AUTHORIZATION_LOCK:
        ...
        if duplicate_id in _V2_RUNTIME_REQUEST_COPY_PROVENANCE:
            _fail("v2 request copy provenance identity was already registered")
        reference = weakref.ref(
            duplicate,
            lambda observed: _drop_v2_request_copy_provenance(
                duplicate_id,
                observed,
            ),
        )
        _V2_RUNTIME_REQUEST_COPY_PROVENANCE[duplicate_id] = reference


def validate_gate_b_v2_reservation_entry(request: GateBLoaderRequest) -> bool:
    """Non-consuming one-shot check for the public loader reservation facade."""
    if not is_gate_b_v2_runtime_request(request):
        return False
    _require_v2_source_only_startup()
    with _V2_RESERVATION_AUTHORIZATION_LOCK:
        ...
        valid = (
            ...
            and _weak_reference_targets(origin, request)
            and type(request._v2_reservation_state) is str
            and _authorization_entry_matches(
                authorization,
                request,
                registered_route.plan
                if type(registered_route) is PreparedGateBV2ExecutionRoute
                else None,
            )
...
def claim_gate_b_v2_reservation_authorization(
    request: GateBLoaderRequest,
    reserve: Callable[[], Any],
) -> Any:
    """Run lower reserve under the one-shot capability and consume it only on success."""
    _require_v2_source_only_startup()
    with _V2_RESERVATION_AUTHORIZATION_LOCK:
        request_id = id(request)
        ...
        if (
            ...
            or not _weak_reference_targets(origin, request)
            or type(request._v2_reservation_state) is not str
            or type(registered_route) is not PreparedGateBV2ExecutionRoute
            or not _authorization_entry_matches(
                authorization,
                request,
                registered_route.plan,
            )
...
def _drop_v2_reservation_authorization(
    ...
    with _V2_RESERVATION_AUTHORIZATION_LOCK:
        ...
        if type(registered) is tuple and registered and registered[0] is reference:
            _V2_RESERVATION_AUTHORIZATIONS.pop(request_id, None)
...
def _drop_v2_request_copy_provenance(
    request_id: int,
    reference: weakref.ReferenceType[GateBLoaderRequest],
) -> None:
    with _V2_RESERVATION_AUTHORIZATION_LOCK:
        if _V2_RUNTIME_REQUEST_COPY_PROVENANCE.get(request_id) is reference:
            _V2_RUNTIME_REQUEST_COPY_PROVENANCE.pop(request_id, None)
...
class PreparedGateBV2ExecutionRoute:
    ...
    def consume(self) -> tuple[GateBLoaderRequest, GateBProductionExecutor]:
        ...
        with _V2_RESERVATION_AUTHORIZATION_LOCK:
            if (
                type(self) is not PreparedGateBV2ExecutionRoute
                or self._closed
                or self._consumed
                or _PREPARED_REGISTRY.get(id(self)) is None
            ):
                _fail(_V2_ROUTE_LIFECYCLE_UNAVAILABLE_ERROR)
...
def prepare_gate_b_v2_execution_route_from_reference(
    ...
    compatibility_chain: GateBV2CompatibilityTrustChain | None = None
    bundle_evidence: ValidatedPhase6ContractBundleEvidence | None = None
    plan: GateBV2ExecutionPlan | None = None
    prepared: PreparedGateBV2ExecutionRoute | None = None
    try:
        _require_v2_source_only_startup()
        ...
        _retire_gate_b_v2_pinned_spec_reference(spec_reference)
        if prepared is None:
            _cleanup_unregistered_v2_plan_graph(
                plan=plan,
                compatibility_chain=compatibility_chain,
                ...
                input_owners=tuple(
                    owner
                    for owner in (spec_owner, input_owner)
                    if type(owner) is _GateBV2RetainedInputOwner
                ),
            )
