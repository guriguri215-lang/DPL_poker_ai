...
select release `0.1.0a13`, and download exactly these four uploaded assets:

- `poker_xai-0.1.0a13-py3-none-any.whl`
- `poker_xai-0.1.0a13.tar.gz`
...
`workflow_dispatch` `tag` and `expected_version` inputs set to `0.1.0a13`. It has
...
assert manifest["version"] == "0.1.0a13"
artifact_names = {
    "poker_xai-0.1.0a13-py3-none-any.whl",
    "poker_xai-0.1.0a13.tar.gz",
}
...
if ($manifest.version -cne '0.1.0a13') { throw 'manifest version mismatch' }
...
The 40 CFR+ iterations remain a fixed alpha computation budget, not a
convergence guarantee.

## Proceed after verification

You may inspect either archive without installing it. For example,
`python3 -I -m zipfile --list poker_xai-0.1.0a13-py3-none-any.whl` lists the
wheel, while `tar -tzf poker_xai-0.1.0a13.tar.gz` lists the sdist. Extracting the
