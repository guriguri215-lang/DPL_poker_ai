...
select release `0.1.0a10`, and download exactly these four uploaded assets:

- `poker_xai-0.1.0a10-py3-none-any.whl`
- `poker_xai-0.1.0a10.tar.gz`
...
`workflow_dispatch` `tag` and `expected_version` inputs set to `0.1.0a10`. It has
...
assert manifest["version"] == "0.1.0a10"
artifact_names = {
    "poker_xai-0.1.0a10-py3-none-any.whl",
    "poker_xai-0.1.0a10.tar.gz",
}
...
if ($manifest.version -cne '0.1.0a10') { throw 'manifest version mismatch' }
...
- `smoke_checks` records version/help commands, entry-point metadata, two
  ...
  `--previous-session-manifest` handoff from the first, saved-bundle verification,
  RunManifest round-trip validation, and documentation relative links.
...
`python3 -I -m zipfile --list poker_xai-0.1.0a10-py3-none-any.whl` lists the
wheel, while `tar -tzf poker_xai-0.1.0a10.tar.gz` lists the sdist. Extracting the
