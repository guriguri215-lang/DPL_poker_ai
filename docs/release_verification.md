...
select release `0.1.0a19`, and download exactly these four uploaded assets:

- `poker_xai-0.1.0a19-py3-none-any.whl`
- `poker_xai-0.1.0a19.tar.gz`
...
`workflow_dispatch` `tag` and `expected_version` inputs set to `0.1.0a19`. It has
...
assert manifest["version"] == "0.1.0a19"
artifact_names = {
    "poker_xai-0.1.0a19-py3-none-any.whl",
    "poker_xai-0.1.0a19.tar.gz",
}
...
if ($manifest.version -cne '0.1.0a19') { throw 'manifest version mismatch' }
...
- `smoke_checks` records version/help commands, entry-point metadata, two
  consecutive Hero sessions on every surface, the second session's explicit
  `--previous-session-manifest` handoff from the first, RunManifest round-trip
  validation, and documentation relative links. Its normal Hero CLI probability
  check rejects `-0.1`, `1.1`, `nan`, and `inf` for both `--safety-alpha` and
  `--exploration-epsilon` with usage and exit code 2, without a traceback,
  reading the deliberately missing previous-session manifest, or creating
  output. Existing successful runs cover in-range explicit values, omitted
  defaults, and saved-settings restoration on all three surfaces. It also
  records a seed-32,
  two-hand `--leaky-fixture --explanations` run that requires an improved
  solver-backed DPL, matching non-empty node-lock solver provenance in its
  explanation, direct verification of every explanation, and successful saved
  explanation-bundle verification. The R008 smoke remains in place. A separate
  seed-20260704, five-hand R007 run uses five solver iterations and explicitly
  passes `--leaky-fixture-reason LEAK_R007 --exploration-epsilon 1.0`; it requires
  the same strict EV improvement and solver-ID/explanation/bundle evidence at the
  bounded OOP `CHECK`/`BET_33` node. R001 and R002 smoke use the same tracked
  0.75-pot equilibrium and the existing nine-opponent development catalogs on
  every surface, with no copied JSON. They require OOP `CHECK`/`BET_75`, causal
  FOLD/CALL observations only after a realized bet, solver-backed exact action
  EV, strict improvement, manifests, post-session evaluation, explanations, and
  saved-bundle verification. R002 also runs a verified successor session that
  explicitly consumes the first manifest, preserves the first bundle, restores
  settings, reselects R002, and proves solver-backed improvement in both
  sessions. On the R001 and R002 saved bundles, every surface also confirms the
  unchanged two-line default verifier output and the explicit
  `--show-evaluation` output: all six stored evaluation metrics, including the
  ...
  confirms restored alpha and epsilon without carrying prior observations or a
  fixture selector. The existing R003 smoke and one-hand handoff remain in place.
...
to facing an all-in; the explicit R007/R003/R004 fixtures add only OOP
...
`python3 -I -m zipfile --list poker_xai-0.1.0a19-py3-none-any.whl` lists the
wheel, while `tar -tzf poker_xai-0.1.0a19.tar.gz` lists the sdist. Extracting the
