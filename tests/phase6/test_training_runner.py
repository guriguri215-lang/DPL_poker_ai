...
def _records(plan):
    def session_records(artifact_type):
        return tuple(
            TrainingArtifactRecord(
                ...
                payload=(
                    payload := _payload(
                        f"{artifact_type}:{key.candidate_id}:{key.opponent_id}:"
                        f"{key.horizon}:{key.repetition_id}"
                    )
                ),
    ...
    def candidate_records(artifact_type):
        return tuple(
            TrainingArtifactRecord(
                candidate.candidate_id,
                _payload_hash(payload := _payload(f"{artifact_type}:{candidate.candidate_id}")),
                payload,
            )
    ...
    return {
        "terminal_candidate_snapshots": session_records("terminal_candidate_snapshots"),
        "hero_policy_snapshots": session_records("hero_policy_snapshots"),
        "exact_ev_cells": session_records("exact_ev_cells"),
        "calibration_cells": candidate_records("calibration_cells"),
        "aggregate_metrics": candidate_records("aggregate_metrics"),
    }
...
def test_writer_emits_canonical_hash_bound_fixture_bundle(plan, tmp_path):
...
def test_writer_rejects_incomplete_or_duplicate_session_joins(plan, tmp_path, artifact_type):
    records = _records(plan)
...
def test_writer_rejects_candidate_loss_extra_types_and_noncanonical_hash(plan, tmp_path):
    records = _records(plan)
...
def test_verifier_rejects_tampering_and_writer_is_immutable(plan, tmp_path):
    records = _records(plan)
    bundle = write_training_artifact_bundle(plan, records, tmp_path)
...
def test_verifier_rehashes_record_payload_after_outer_reference_rehash(plan, tmp_path):
    bundle = write_training_artifact_bundle(plan, _records(plan), tmp_path)
...
def test_verifier_rejects_valid_but_unapproved_stream_root(plan, tmp_path, substitution):
    bundle = write_training_artifact_bundle(plan, _records(plan), tmp_path)
...
def test_verifier_rejects_coherently_reidentified_candidate(plan, tmp_path):
    bundle = write_training_artifact_bundle(plan, _records(plan), tmp_path)
    name = "training_batch_manifest"
    manifest = json.loads((tmp_path / bundle.references[name]["path"]).read_bytes())
