...
def test_manifest_is_valid_and_pins_versions_and_configs():
    ...
    assert manifest.code.package_version == "0.1.0a6"
...
def test_session_version_commands_exit_without_starting_or_writing(tmp_path, monkeypatch, capsys):
    ...
    assert capsys.readouterr().out == "poker-xai-run-session 0.1.0a6\n"
    ...
    assert capsys.readouterr().out == "cli/run_session.py 0.1.0a6\n"
    assert tuple(tmp_path.iterdir()) == ()
