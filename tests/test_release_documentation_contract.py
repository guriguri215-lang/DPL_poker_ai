...
def test_release_notes_contract_covers_scope_and_alpha_limitations() -> None:
    runbook = _normalized("docs/releasing.md")
    for required in (
        "manifest-first offline revalidation",
        "read-only pure API",
        "thin distribution CLI",
        "ArtifactRef",
        "relative-path and SHA-256 checks",
        "count/order/session/hand pairing",
        "existing explanation checker",
        "verifier-summary consistency",
        "share pairing, checker-result, and summary decisions",
        "PR #18 through PR #20",
        "snapshot-bytes, canonical-JSON, schema/type, complete-shape",
        "operator alpha/epsilon flags, including `0.0`, override restored values",
        "before a Hero session starts or output exists",
        "immediately preceding saved-bundle format",
        "no dependency, network input, LLM, solver or session execution",
        ...
        "Explanation schemas",
        "simulation-only",
        "no implicit manifest discovery, registry, or automatic session loop",
