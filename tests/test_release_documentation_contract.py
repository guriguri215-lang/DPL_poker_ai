...
def test_release_notes_contract_covers_scope_and_alpha_limitations() -> None:
    runbook = _normalized("docs/releasing.md")
    for required in (
        "Post-session validation parity",
        "saved explanation-bundle revalidation",
        "explicit next-session settings loading",
        "captured-snapshot, schema/type, complete-shape, identity",
        "Operator alpha/epsilon flags, including `0.0`, override restored values",
        "before a Hero session starts or output exists",
        "Multiple-record evaluation uses every DPL record",
        "eligible and reached populations",
        "aggregates exact decision EV and over-/under-adjustment counts",
        "count, order, session, and hand",
        "do not depend on record order",
        "Reason/situation binding",
        "specific situation",
        "restores detector configuration, safety alpha, and exploration epsilon",
        "no implicit manifest discovery, registry, automatic session loop",
        "inheritance of baseline, history, or answer-key evidence",
        "Normal Hero `--leaky-fixture` sessions use the real node-lock provider",
        "existing rule fallback",
        "strictly improves exact decision EV",
        "default non-leaky baseline",
        "P5-4 uses the same node-lock provider and fallback",
        '`exploit_source="nodelock_solver"`',
        "non-empty `solver_result_id`",
        "the same ID appears in its explanation",
        "every explanation passes the existing verifier",
        "seed-32, two-hand leaky-fixture run",
        "solver-backed provenance",
        "saved explanation bundle",
        "`--leaky-fixture-reason LEAK_R007`",
        "Bare `--leaky-fixture` still selects R008",
        "OOP no-facing `CHECK`/`BET_33` decision",
        "zero-opportunity terminal scope",
        "seed-20260704, five-hand, five-iteration smoke",
        "`--exploration-epsilon 1.0` explicitly",
        "R008 smoke",
        "no new dependency, entry point, schema, solver public API, workflow topology",
        "no `BET_75` or raise branch",
